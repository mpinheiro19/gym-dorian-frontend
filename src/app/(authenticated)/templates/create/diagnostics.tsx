...
export function ExerciseDiagnostics() {
  ...
  return (
    <div className="bg-warning-surface border border-warning rounded p-4 mb-4">
      <h3 className="font-semibold text-warning-text mb-2">🔍 Diagnostics</h3>
      <div className="text-sm space-y-1">
        ...
        {error && (
          <p className="text-error">Error: {(error as any)?.message || 'Unknown error'}</p>
        )}
      </div>
    </div>
  );
